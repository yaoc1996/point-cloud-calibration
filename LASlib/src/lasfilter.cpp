...
public:
  ...
  {
    int n = 0;
    n += one->get_command(&string[n]);
    n += two->get_command(&string[n]);
    n += sprintf(&string[n], "-%s ", name());
    return n;
  };
  ...
  LAScriterionAnd(LAScriterion *one, LAScriterion *two)
  {
    this->one = one;
    this->two = two;
  };
...
public:
  ...
  inline U32 get_decompress_selective() const { return (one->get_decompress_selective() | two->get_decompress_selective()); };
  inline BOOL filter(const LASpoint *point) { return one->filter(point) || two->filter(point); };
  LAScriterionOr(LAScriterion *one, LAScriterion *two)
  {
    this->one = one;
    this->two = two;
  };
...
public:
  ...
  LAScriterionKeepTile(F32 ll_x, F32 ll_y, F32 tile_size)
  {
    this->ll_x = ll_x;
    this->ll_y = ll_y;
...
public:
  ...
  LAScriterionKeepCircle(F64 x, F64 y, F64 radius)
  {
    this->center_x = x;
    this->center_y = y;
    this->radius = radius;
...
public:
  ...
  LAScriterionKeepxyz(F64 min_x, F64 min_y, F64 min_z, F64 max_x, F64 max_y, F64 max_z)
  {
    this->min_x = min_x;
    this->min_y = min_y;
    this->min_z = min_z;
...
public:
  ...
  inline U32 get_decompress_selective() const { return LASZIP_DECOMPRESS_SELECTIVE_CHANNEL_RETURNS_XY | LASZIP_DECOMPRESS_SELECTIVE_Z; };
  inline BOOL filter(const LASpoint *point) { return (point->inside_box(min_x, min_y, min_z, max_x, max_y, max_z)); };
  LAScriterionDropxyz(F64 min_x, F64 min_y, F64 min_z, F64 max_x, F64 max_y, F64 max_z)
  {
    this->min_x = min_x;
    this->min_y = min_y;
    this->min_z = min_z;
...
public:
  ...
  LAScriterionKeepxy(F64 below_x, F64 below_y, F64 above_x, F64 above_y)
  {
...
public:
  ...
  inline I32 get_command(CHAR *string) const { return sprintf(string, "-%s %lf %lf %lf %lf ", name(), below_x, below_y, above_x, above_y); };
  inline BOOL filter(const LASpoint *point) { return (point->inside_rectangle(below_x, below_y, above_x, above_y)); };
  LAScriterionDropxy(F64 below_x, F64 below_y, F64 above_x, F64 above_y)
  {
    this->below_x = below_x;
    this->below_y = below_y;
    this->above_x = above_x;
...
public:
  ...
  {
    F64 x = point->get_x();
    return (x < below_x) || (x >= above_x);
  };
  LAScriterionKeepx(F64 below_x, F64 above_x)
  {
    this->below_x = below_x;
    this->above_x = above_x;
  };
...
public:
  ...
  {
    F64 x = point->get_x();
    return ((below_x <= x) && (x < above_x));
  };
  LAScriterionDropx(F64 below_x, F64 above_x)
  {
    this->below_x = below_x;
    this->above_x = above_x;
  };
...
public:
  ...
  {
    F64 y = point->get_y();
    return (y < below_y) || (y >= above_y);
  };
  LAScriterionKeepy(F64 below_y, F64 above_y)
  {
    this->below_y = below_y;
    this->above_y = above_y;
  };
...
public:
  ...
  {
    F64 y = point->get_y();
    return ((below_y <= y) && (y < above_y));
  };
  LAScriterionDropy(F64 below_y, F64 above_y)
  {
    this->below_y = below_y;
    this->above_y = above_y;
  };
...
public:
  ...
  {
    F64 z = point->get_z();
    return (z < below_z) || (z >= above_z);
  };
  LAScriterionKeepz(F64 below_z, F64 above_z)
  {
...
public:
  ...
  {
    F64 z = point->get_z();
    return ((below_z <= z) && (z < above_z));
  };
  LAScriterionDropz(F64 below_z, F64 above_z)
  {
    this->below_z = below_z;
    this->above_z = above_z;
  };
...
public:
  ...
  LAScriterionKeepXY(I32 below_X, I32 below_Y, I32 above_X, I32 above_Y)
  {
    this->below_X = below_X;
    this->below_Y = below_Y;
    this->above_X = above_X;
...
public:
  ...
  LAScriterionKeepX(I32 below_X, I32 above_X)
  {
    this->below_X = below_X;
    this->above_X = above_X;
  };
...
public:
  ...
  LAScriterionDropX(I32 below_X, I32 above_X)
  {
    this->below_X = below_X;
    this->above_X = above_X;
  };
...
public:
  ...
  LAScriterionKeepY(I32 below_Y, I32 above_Y)
  {
    this->below_Y = below_Y;
    this->above_Y = above_Y;
  };
...
public:
  ...
  LAScriterionDropY(I32 below_Y, I32 above_Y)
  {
    this->below_Y = below_Y;
    this->above_Y = above_Y;
  };
...
public:
  ...
  LAScriterionKeepZ(I32 below_Z, I32 above_Z)
  {
    this->below_Z = below_Z;
    this->above_Z = above_Z;
  };
...
public:
  ...
  LAScriterionDropZ(I32 below_Z, I32 above_Z)
  {
    this->below_Z = below_Z;
    this->above_Z = above_Z;
  };
...
public:
  ...
  inline I32 get_command(CHAR *string) const
  {
    ...
    for (i = 0; i < 16; i++)
      if ((1 << i) & keep_return_mask)
        n += sprintf(string + n, "%u ", i);
    return n;
  };
...
public:
  ...
  inline BOOL filter(const LASpoint *point)
  {
    if (scan_direction_flag == point->scan_direction_flag)
      return TRUE;
    I32 s = scan_direction_flag;
...
public:
  ...
  LAScriterionKeepRGB(I32 below_RGB, I32 above_RGB, I32 channel)
  {
...
public:
  ...
  inline BOOL filter(const LASpoint *point)
  {
    I32 greenness = 2 * point->get_G() - point->get_R() - point->get_B();
    return ((greenness < below_RGB) || (above_RGB < greenness));
  };
  LAScriterionKeepRGBgreenness(I32 below_RGB, I32 above_RGB)
  {
    if (above_RGB < below_RGB)
    {
      this->below_RGB = above_RGB;
...
public:
  ...
  LAScriterionKeepNDVI(F32 below_NDVI, F32 above_NDVI, I32 NIR)
  {
    if (above_NDVI < below_NDVI)
    {
      this->below_NDVI = above_NDVI;
      this->above_NDVI = below_NDVI;
    }
    else
    {
      this->below_NDVI = below_NDVI;
      this->above_NDVI = above_NDVI;
    };
    this->NIR = NIR;
  };
...
public:
  ...
  LAScriterionKeepNDVIfromCIR(F32 below_NDVI, F32 above_NDVI)
  {
    if (above_NDVI < below_NDVI)
    {
      this->below_NDVI = above_NDVI;
...
public:
  ...
  LAScriterionKeepNDVIintensityIsNIR(F32 below_NDVI, F32 above_NDVI)
  {
    if (above_NDVI < below_NDVI)
    {
      this->below_NDVI = above_NDVI;
...
public:
  ...
  LAScriterionKeepIntensity(U16 below_intensity, U16 above_intensity)
  {
    this->below_intensity = below_intensity;
    this->above_intensity = above_intensity;
  };
...
public:
  ...
  inline I32 get_command(CHAR *string) const
  {
    ...
    for (i = 0; i < 32; i++)
      if ((1u << i) & keep_classification_mask)
        n += sprintf(string + n, "%u ", i);
    return n;
  };
...
public:
  ...
  inline I32 get_command(CHAR *string) const
  {
    ...
    for (i = 0; i < 32; i++)
      if ((1 << i) & drop_classification_mask)
        n += sprintf(string + n, "%u ", i);
    return n;
  };
  inline U32 get_decompress_selective() const { return LASZIP_DECOMPRESS_SELECTIVE_CLASSIFICATION; };
...
public:
  ...
  LAScriterionDropExtendedClassifications(U32 drop_extended_classification_mask[8])
  {
    for (I32 i = 0; i < 8; i++)
      this->drop_extended_classification_mask[i] = drop_extended_classification_mask[i];
  };
...
public:
  ...
  LAScriterionKeepUserDataBetween(U8 below_user_data, U8 above_user_data)
  {
    this->below_user_data = below_user_data;
    this->above_user_data = above_user_data;
  };
...
public:
  ...
  LAScriterionDropUserDataBetween(U8 below_user_data, U8 above_user_data)
  {
    this->below_user_data = below_user_data;
    this->above_user_data = above_user_data;
  };
...
public:
  ...
  LAScriterionKeepPointSourceBetween(U16 below_point_source_id, U16 above_point_source_id)
  {
...
public:
  ...
  LAScriterionDropPointSourceBetween(U16 below_point_source_id, U16 above_point_source_id)
  {
    this->below_point_source_id = below_point_source_id;
    this->above_point_source_id = above_point_source_id;
  };
...
public:
  ...
  LAScriterionKeepAttributeBelow(U32 index, F64 below_attribute)
  {
    this->index = index;
    this->below_attribute = below_attribute;
  };
...
public:
  ...
  LAScriterionKeepAttributeAbove(U32 index, F64 above_attribute)
  {
    this->index = index;
    this->above_attribute = above_attribute;
  };
...
public:
  ...
  inline BOOL filter(const LASpoint *point)
  {
    ...
    return (attribute < below_attribute) || (above_attribute < attribute);
  };
  LAScriterionKeepAttributeBetween(U32 index, F64 below_attribute, F64 above_attribute)
  {
    this->index = index;
    this->below_attribute = below_attribute;
    this->above_attribute = above_attribute;
...
public:
  ...
  inline U32 get_decompress_selective() const { return LASZIP_DECOMPRESS_SELECTIVE_EXTRA_BYTES; };
  inline BOOL filter(const LASpoint *point) { return (point->get_attribute_as_float(index) < below_attribute); };
  LAScriterionDropAttributeBelow(U32 index, F64 below_attribute)
  {
    this->index = index;
    this->below_attribute = below_attribute;
  };
...
public:
  ...
  inline U32 get_decompress_selective() const { return LASZIP_DECOMPRESS_SELECTIVE_EXTRA_BYTES; };
  inline BOOL filter(const LASpoint *point) { return (point->get_attribute_as_float(index) > above_attribute); };
  LAScriterionDropAttributeAbove(U32 index, F64 above_attribute)
  {
    this->index = index;
    this->above_attribute = above_attribute;
  };
...
public:
  ...
  {
    F64 attribute = point->get_attribute_as_float(index);
    return (below_attribute <= attribute) && (attribute <= above_attribute);
  };
  LAScriterionDropAttributeBetween(U32 index, F64 below_attribute, F64 above_attribute)
  {
    this->index = index;
    this->below_attribute = below_attribute;
    this->above_attribute = above_attribute;
...
public:
  ...
  LAScriterionKeepEveryNth(U32 every)
  {
    this->every = every;
    counter = 1;
  };
...
public:
  ...
  LAScriterionDropEveryNth(U32 every)
  {
    this->every = every;
    counter = 1;
  };
...
public:
  ...
  {
    requested_seed = seed = 0;
    this->fraction = fraction;
  };
  LAScriterionKeepRandomFraction(U32 seed, F32 fraction)
  {
    requested_seed = this->seed = seed;
    this->fraction = fraction;
  };
...
public:
  ...
  inline BOOL filter(const LASpoint *point)
  {
    ...
    if ((U32)pos_y >= *array_size)
    {
      ...
      if (*array_size)
      {
        if (array == &minus_plus || array == &plus_plus)
          *ankers = (I32 *)realloc(*ankers, array_size_new * sizeof(I32));
      ...
      {
        if (array == &minus_plus || array == &plus_plus)
          *ankers = (I32 *)malloc(array_size_new * sizeof(I32));
    ...
    if ((*array)[pos_y][pos_x_pos] & pos_x_bit)
      return TRUE;
  ...
  void reset()
  {
    ...
    if (minus_minus_size)
    {
      for (U32 i = 0; i < minus_minus_size; i++)
        if (minus_minus[i])
          free(minus_minus[i]);
      free(minus_minus);
      minus_minus = 0;
    ...
    if (minus_plus_size)
    {
      free(minus_ankers);
      minus_ankers = 0;
      for (U32 i = 0; i < minus_plus_size; i++)
        if (minus_plus[i])
          free(minus_plus[i]);
      free(minus_plus);
      minus_plus = 0;
    ...
    if (plus_minus_size)
    {
      for (U32 i = 0; i < plus_minus_size; i++)
        if (plus_minus[i])
          free(plus_minus[i]);
      free(plus_minus);
      plus_minus = 0;
    ...
    if (plus_plus_size)
    {
      free(plus_ankers);
      plus_ankers = 0;
      for (U32 i = 0; i < plus_plus_size; i++)
        if (plus_plus[i])
          free(plus_plus[i]);
      free(plus_plus);
      plus_plus = 0;
