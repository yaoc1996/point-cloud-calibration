...
BOOL LASquadtree::coarsen(const I32 cell_index, I32 *coarser_cell_index, U32 *num_cell_indices, I32 **cell_indices)
{
  ...
  if (coarser_cell_index)
    (*coarser_cell_index) = get_cell_index(level_index, level - 1);
...
{
  int level = 0;
  while (cell_index >= level_offset[level + 1])
    level++;
  return level;
}
...
BOOL LASquadtree::read(ByteStreamIn *stream)
{
  ...
  try
  {
    stream->get32bitsLE((U8 *)&level_index);
  }
  catch (...)
  {
  ...
  try
  {
    stream->get32bitsLE((U8 *)&implicit_levels);
  }
  catch (...)
  {
...
BOOL LASquadtree::manage_cell(const U32 cell_index, const BOOL finalize)
{
  ...
  if (adaptive_pos >= adaptive_alloc)
  {
    if (adaptive)
    {
      ...
      for (U32 i = adaptive_alloc; i < adaptive_pos * 2; i++)
        adaptive[i] = 0;
    ...
    else
    {
      ...
      for (U32 i = adaptive_alloc; i <= adaptive_pos; i++)
        adaptive[i] = 0;
...
BOOL LASquadtree::setup(F64 bb_min_x, F64 bb_max_x, F64 bb_min_y, F64 bb_max_y, F32 cell_size)
{
  ...
  if (bb_min_x >= 0)
    min_x = cell_size * ((I32)(bb_min_x / cell_size));
  else
    min_x = cell_size * ((I32)(bb_min_x / cell_size) - 1);
  if (bb_max_x >= 0)
    max_x = cell_size * ((I32)(bb_max_x / cell_size) + 1);
  else
    max_x = cell_size * ((I32)(bb_max_x / cell_size));
  if (bb_min_y >= 0)
    min_y = cell_size * ((I32)(bb_min_y / cell_size));
  else
    min_y = cell_size * ((I32)(bb_min_y / cell_size) - 1);
  if (bb_max_y >= 0)
    max_y = cell_size * ((I32)(bb_max_y / cell_size) + 1);
  else
    max_y = cell_size * ((I32)(bb_max_y / cell_size));
...
BOOL LASquadtree::setup(F64 bb_min_x, F64 bb_max_x, F64 bb_min_y, F64 bb_max_y, F32 cell_size, F32 offset_x, F32 offset_y)
{
  ...
  if ((bb_min_x - offset_x) >= 0)
    min_x = cell_size * ((I32)((bb_min_x - offset_x) / cell_size)) + offset_x;
  else
    min_x = cell_size * ((I32)((bb_min_x - offset_x) / cell_size) - 1) + offset_x;
  if ((bb_max_x - offset_x) >= 0)
    max_x = cell_size * ((I32)((bb_max_x - offset_x) / cell_size) + 1) + offset_x;
  else
    max_x = cell_size * ((I32)((bb_max_x - offset_x) / cell_size)) + offset_x;
  if ((bb_min_y - offset_y) >= 0)
    min_y = cell_size * ((I32)((bb_min_y - offset_y) / cell_size)) + offset_y;
  else
    min_y = cell_size * ((I32)((bb_min_y - offset_y) / cell_size) - 1) + offset_y;
  if ((bb_max_y - offset_y) >= 0)
    max_y = cell_size * ((I32)((bb_max_y - offset_y) / cell_size) + 1) + offset_y;
  else
    max_y = cell_size * ((I32)((bb_max_y - offset_y) / cell_size)) + offset_y;
...
{
  if (current_cells)
    delete ((my_cell_vector *)current_cells);
  if (adaptive)
    free(adaptive);
}
